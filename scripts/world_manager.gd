...
## Create a new ship seeded with a cockpit, sitting at `pos` and oriented so its
## up = `up` and forward (-Z) = `fwd`. Returns the ship.
func spawn_ship(pos: Vector3, up: Vector3, fwd: Vector3) -> Ship:
	var ship := Ship.new()
	ship.world = self
	add_child(ship)
	ship.global_position = pos
	if fwd.cross(up).length() > 0.01:
		ship.look_at(pos + fwd, up)
