...
func _build_ground() -> void:
	...
	m.albedo_color = Color(0.20, 0.23, 0.20)
	m.roughness = 1.0
...
func _add_fire(at: Vector3) -> void:
	...
	p.mesh = BoxMesh.new()
	(p.mesh as BoxMesh).size = Vector3(0.5, 0.5, 0.5)
...
func _pose(t: float) -> void:
	...
	if _impacting:
		# Once the world is ready she stops looping and goes in.
		var k: float = clampf(_ending / IMPACT_TIME, 0.0, 1.0)
		y = lerpf(20.0, GROUND_Y + 6.0, k * k)
	...
	var dist: float = 26.0 - 8.0 * sin(t * 0.2)
	if _impacting:
		# Pulls out and back for the last second, so the ground is in the shot.
		var k2: float = clampf(_ending / IMPACT_TIME, 0.0, 1.0)
		dist = lerpf(dist, 52.0, k2)
	var eye: Vector3 = _ship.position + Vector3(
		cos(ang) * dist, 6.0 + 4.0 * sin(t * 0.45), sin(ang) * dist)
	_cam.look_at_from_position(eye, _ship.position, Vector3.UP)
