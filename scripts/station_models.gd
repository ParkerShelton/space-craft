...
const FOOTPRINT := {
	...
	Blocks.POWER_BAY: Vector3i(2, 1, 1),
...
static func boxes_for(kind: int) -> Array:
	match kind:
		...
		Blocks.POWER_BAY:
			var out3: Array = []
			out3.append([Vector3(0, 0.12, 0), Vector3(1.94, 0.24, 0.9), DARK])
			for i in 3:
				out3.append([Vector3(-0.62 + i * 0.62, 0.5, 0), Vector3(0.5, 0.6, 0.72), METAL])
				out3.append([Vector3(-0.62 + i * 0.62, 0.84, 0), Vector3(0.36, 0.1, 0.5), GLOW])
			return out3
		Blocks.OXYGEN_PLANT:
			return [
