...
## Kept so any caller left over is harmless, and so a station part way through
## being taken apart when you let go springs back to its proper size. Coming
## apart is shown with the same cracking overlay a block gets (see
## Player._crack_station) -- it no longer shrinks and reddens, which was a
## language nothing else in the game spoke.
func set_dismantle(_t: float) -> void:
	if _mi != null and _mi.scale != Vector3.ONE:
